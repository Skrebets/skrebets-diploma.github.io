...
let resultArray = JSON.parse(localStorage.resultArray); // Получаем result из localStorage

let keyWord = localStorage.keyWord; // Получаем ключевое слово из localStorage
...
let nowTimestamp = +new Date(); // Сегодняшняя дата в timestamp


/* Даты за numberOfDays дней до текущей даты */


let nowOneAgo = getDayAgo(1, nowTimestamp);

let nowTwoAgo = getDayAgo(2, nowTimestamp);

let nowThreeAgo = getDayAgo(3, nowTimestamp);

let nowFourAgo = getDayAgo(4, nowTimestamp);

let nowFiveAgo = getDayAgo(5, nowTimestamp);

let nowSixAgo = getDayAgo(6, nowTimestamp);
